...
object Anagrams {
  ...
  def wordOccurrences(word: Word): Occurrences =
    word.groupBy(_.toLower).toList.map { case(c, l) => (c, l.size) } .sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsHelper(char: Char, count: Int) = for (i <- 1 to count) yield (char, i)

    occurrences match {
      case Nil => Nil :: Nil

      case (char, count) :: tail => {
        val right = combinations(tail)

        (for {
          r <- right
          l <- combinationsHelper(char, count)
        }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    @tailrec
    def impl(x: Occurrences, y: Occurrences, acc: Occurrences = Nil): Occurrences = {
      (x, y) match {
        case ((xchar, xcount) :: xs, (ychar, ycount) :: ys) => xchar.compare(ychar).signum match {
          case -1 => impl(xs, y, x.head :: acc)
          case 1 => impl(x, ys, y.head :: acc)

          case _ => xcount - ycount match {
            case 0 => impl(xs, ys, acc)
            case count =>  impl(xs, ys, (xchar, count) :: acc)
          }
        }

        case _ => acc.reverse ++ x ++ y
      }
    }
    ...
    def impl(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          ...
          tail <- impl(subtract(occurrences, combination))
        }
        yield word :: tail
      }
    }
