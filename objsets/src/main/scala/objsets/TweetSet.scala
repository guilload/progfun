...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweetHelper(list: TweetList): TweetList = {
    val max = this.mostRetweeted
    val subset = this.remove(max)
    new Cons(max, subset.descendingByRetweet)
  }
  ...
  def mostRetweetedHelper(currentMax: Tweet): Tweet = {
    val maxLeft = left.mostRetweetedHelper(currentMax)
    val maxRight = right.mostRetweetedHelper(maxLeft)

    if (maxRight.retweets < elem.retweets)
      elem
    else
      maxRight
  }
